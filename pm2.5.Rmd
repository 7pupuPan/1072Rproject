...
pm_regionCode_list <- pm_importWorksheets(filename = 
                                            "C:/Users/ASUS/Desktop/workPlace/Taiwan_Geocode_103_v2.xlsx")

pm_regionCode_county_df <- pm_regionCode_list[[1]]
...
turn county into code
```{r}
for(i in 1:nrow(pm_death_total)){
  for(j in 1:nrow(pm_regionCode_county_df)){
    if(pm_death_total[i, 1] == pm_regionCode_county_df[j, 2]){
      pm_death_total[i, 1] <- pm_regionCode_county_df[j, 3]
    }
  }
}
...
for(i in 1:nrow(pm_death2_total)){
  for(j in 1:nrow(pm_regionCode_county_df)){
    if(pm_death2_total[i, 1] == pm_regionCode_county_df[j, 2]){
      pm_death2_total[i, 1] <- pm_regionCode_county_df[j, 3]
    }
  }
}
```
