...
#  reg<-lm(formula = log(kWh) ~ PM2.5,data=pm_ce_year)
# summary(reg)
```
...
pm_ce_month<-pm_withcodes%>%
  ...
  rename(distinct=distinct.x,county=county.x)%>%
  arrange(county_code)
pm_ce_month
